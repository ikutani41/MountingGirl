var BackgroundLayer = cc.Layer.extend({
    ...
    init:function () {
        ...
        cc.log("Lv:"+lv+",Exp:"+exp);

        // set game-config
        this.Goal = goalSteps[lv-1];
        this.Answer = [];   //初期化
        this.Fucknum = goalSteps[lv-1]+1;
        ...
        if(lv == 1){
            var instruction = new cc.Sprite(res.Instruction_png);
            instruction.setPosition(cc.p(w/2,scrollY*1.5));
            instruction.setOpacity(80);
    ...
    touched: function (posX){
        ...
        if(w/4 * num < posX & posX < w/4 * (num + 1)){
            ...
            if(this.Step % 40 == 0){
                var recordStep = this.Goal - this.Step;
                var recordTime = statusLayer.getTimeLeft();
                this.getParent().setRecord(recordStep,recordTime);
                cc.log("recorded!");
            }
        } else {
            //不正解のとき
    ...
    addImage:function(){
        ...
        if(this.Ready < this.Goal){
            ...
            if(this.Step % 40 == 3 && this.Saveline == 1){
